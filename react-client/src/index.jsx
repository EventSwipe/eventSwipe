...
class App extends React.Component {
  ...
  searchEvents(query) {
    this.setState({ loading: true }, () => {
      axios
        .get('/events', query) // axios get request from API and then populates the events state with the data
        .then(({ data }) => {
  ...
  render() {
    ...
    const showFavesOrEvents = showFaves ? (
      <Favorites user={user} />
    ) : (
      <SearchBar
        events={events}
    ...
    return this.state.authenticated ? (
      <container>
        <button
          ...
          onClick={() => this.setState({ showFaves: !showFaves })}
      ...
      <div id="login_div" className="main-div">
        <img
          ...
          src="https://image.ibb.co/dQsGup/coollogo_com_30844732.png"
        />
        <br />
        <button
